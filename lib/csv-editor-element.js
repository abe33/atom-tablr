...
class CSVEditorElement extends HTMLElement {
  ...
  focus () { return this.tableElement && this.tableElement.focus() }
  ...
  setModel (model) {
    ...
    this.subscriptions.add(this.model.onDidChange(() => {
      if (this.model.editor != null) {
        if (this.model.editor !== this.tableElement.getModel()) {
          ...
          return this.displayTableEditor(this.model.editor)
        }
      } else if (this.tableElement != null) {
        return this.createFormView()
      } else {
        return this.updatePreview(this.collectOptions())
      }
    }))
    ...
    this.subscriptions.add(this.model.onWillOpen(() => {
      this.ensureProgress()
      this.removeFormView()
      loadingSubscription = this.subscriptions.add(this.model.onDidReadData(({input, lines}) => {
        this.input = input
        this.lines = lines
    ...
    this.subscriptions.add(this.model.onWillFillTable(() => {
      ...
      loadingSubscription = this.subscriptions.add(this.model.onFillTable(({table}) => {
  ...
  createFormView () {
    ...
    if (this.model != null) { this.form.setModel(this.model.options) }
    this.appendChild(this.formContainer)
  }

  removeFormView () {
    if (this.formContainer) { this.removeChild(this.formContainer) }
    delete this.form
  ...
  updatePreview (options) {
    ...
    return this.model.previewCSV(options).then(preview => {
      if (this.form == null) { return }
      this.form.preview.render(preview, options)
      return this.form.openTableEditorButton.removeAttribute('disabled')
    })
    .catch(reason => {
      if (this.form == null) { return }
      this.form.preview.error(reason)
      return this.form.openTableEditorButton.setAttribute('disabled', 'true')
    })
  }

  ensureProgress () {
    if (this.progress == null) { return this.displayProgress() }
  }
  ...
  requestProgressUpdate () {
    if (this.frameRequested || (this.progress == null)) { return }
