...
class TableElement extends HTMLElement {
  ...
  startColumnResizeDrag ({pageX, target}) {
    ...
    const position = parseInt(cellElement.dataset.column)
  ...
  scrollDuringDrag (row, column) {
    ...
    if (column != null) {
      ...
      const columnOffset = this.tableEditor.getScreenColumnOffsetAt(row)
      const columnWidth = this.tableEditor.getScreenColumnWidthAt(row)
